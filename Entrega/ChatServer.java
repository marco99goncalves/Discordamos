...
public class ChatServer {
  ...
  static void SendMessageToAllButSender(
    String message,
    Room room,
    SelectionKey receiverKey
  ) {
    if (room == null) return;

    for (ClientModel client : room.clients) {
      SelectionKey key = client.getKey();
      if (key == receiverKey) continue; // Ignore the sender
  ...
  static void RunByeCommand(
    ...
    if (sendBye) SendMessageToUser("BYE\n", senderKey);
